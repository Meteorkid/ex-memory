...
def test_build_manifest_records_every_allowed_store_origin(tmp_path: Path):
    ...
    assert result.returncode == 0
    manifest = json.loads(output.read_text(encoding="utf-8"))
    assert manifest["site_origins"] == ["https://imagentx.top", "https://www.imagentx.top"]
...
def test_build_script_bundles_every_explicit_store_origin():
    ...
    assert 'printf \'%s\\n\' "${SITE_ORIGIN_LIST[@]}" > "$BUILD_ROOT/release-origin.txt"' in script
...
def test_build_script_does_not_expose_local_path_in_checksum_file():
    ...
    assert '(cd "$OUTPUT_ROOT" && shasum -a 256 "$DMG_NAME" > "$DMG_NAME.sha256")' in script
