...
def test_read_contacts_and_sessions_v4(tmp_path: Path):
    ...
    with sqlite3.connect(session_db) as connection:
        ...
        connection.execute(
            "INSERT INTO SessionTable VALUES (?, ?, ?)", ("wxid_friend", 123, 9)
        )
        connection.execute(
            "INSERT INTO SessionTable VALUES (?, ?, ?)", ("filehelper", 124, 1)
        )
...
def test_contact_and_session_schema_optional_columns(tmp_path: Path):
    ...
    with sqlite3.connect(session_db) as connection:
        connection.execute(
            "CREATE TABLE SessionTable(username TEXT, last_time INTEGER)"
        )
...
def test_iter_messages_v4_orders_shards_and_marks_sender_id_two_as_self(tmp_path: Path):
    ...
    for index, rows in enumerate((((2, 200, b"second"),), ((1, 100, b"first"),))):
        ...
        with sqlite3.connect(database) as connection:
            ...
            connection.execute(
                "INSERT INTO Name2Id(rowid, user_name) VALUES (1, ?)", (owner_wxid,)
            )
            connection.execute(
                "INSERT INTO Name2Id(rowid, user_name) VALUES (2, ?)", (session_wxid,)
            )
            connection.execute(
    ...
    messages = tuple(
        iter_messages(shards, session_wxid=session_wxid, owner_wxid=owner_wxid)
    )
...
def test_group_sender_prefix_and_app_subtype(tmp_path: Path):
    ...
    with sqlite3.connect(database) as connection:
        ...
        connection.execute(
            f'INSERT INTO "{table}" VALUES (1, 49, 100, ?)', (content.encode(),)
        )

    message = next(
        iter_messages((database,), session_wxid=session_wxid, owner_wxid=owner_wxid)
    )
