...
def test_create_flow_api_success():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir = Path(tmpdir)
        with (
            patch("pipeline.orchestrator.get_ex_dir", return_value=tmpdir),
            ...
            patch(
                "pipeline.orchestrator.get_llm_config",
                return_value={"api_key": "test-key", "model": "test"},
            ),
            patch(
                "pipeline.orchestrator.get_collection_name",
                return_value="test_collection",
            ),
            patch(
                "pipeline.orchestrator.build_memory", return_value="# Memory\n测试记忆"
            ),
            patch(
                "pipeline.orchestrator.build_persona",
...
def test_create_flow_api_memory_failure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with (
            ...
            patch(
                "pipeline.orchestrator.build_memory",
            ...
            result = run_create_flow_api(slug="test", name="测试", answers=["info"])
            assert "error" in result
